...
```{r}
mlr <- lm(log_totalcost ~ ERvisits*I(comp_bin) , data = heart_data)

broom::tidy(mlr) %>% view()
```

`comp_bin` is significant, as well as `ERvisits` and the intercept `log_totalcost`, but the interaction between `ERvisits` and `comp_bin` is significant. 

Since `comp_bin` is significant, it is an effect modifier.
...
confounder_model <- lm(log_totalcost ~ ERvisits + I(comp_bin), data = heart_data)
...
The interaction between `comp_bin` and `ERvists` is significant, so this term should be included, but it doesn't significantly change the slope of `ERvisits` when added as a cofounder, so it should not be included as a cofounder. 
...
```{r}
final_model <- lm(log_totalcost ~ ERvisits + I(comp_bin) + age + I(gender) + duration, data = heart_data)
summary(final_model)
